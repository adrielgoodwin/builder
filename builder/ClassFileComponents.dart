...
class ClassFileComponents {
  ...
  ClassFileComponents(this.classData);
  ...
  void generateHead() {
    head.add(
        "// This code was generated on ${DateTime.now().toString().substring(0, 16)}"); // TODO make this pretty please Aylin
    head.add("// class");
  ...
  void generateConstructorFunction() {
    ...
    constructor = [
      constructorFunc.substring(0, constructorFunc.length - 2) + "});"
    ];
  }
  ...
  void generateFromJsonConstructor() {
    ...
    String functionHead =
        '  factory ${classData.name}.fromJson(Map<String, dynamic> data) {';
    ...
    classData.fieldData.forEach((fieldData) {
      ...
      if (fieldData.isAClass) {
        if (fieldData.isAList) {
          ...
          functionBody
              .add("    var ${fieldName.paramify()}List = data['$fieldName'];");
          functionBody
              .add("    List<$fieldType> listOf${fieldName.classify()} = [];");
          functionBody.add(
              "    for(var i = 0; i < ${fieldName.paramify()}List.length; i++){");
          functionBody.add(
              "      listOf${fieldName.classify()}.add($fieldType.fromJson(${fieldName.paramify()}List[i]));");
          functionBody.add("    }");
          // then reference this newly made list in the return
          // eg. supplies: listOfSupplies,
          functionReturn.add(
              "      ${fieldName.paramify()}: listOf${fieldName.classify()},");
        } else {
          // if its a class, but not a list no addition to function body, only to return

          // eg. classData: ClassData.fromJson(data['classData']),
          functionReturn.add(
              "      ${fieldName.paramify()}: ${fieldName.classify()}.fromJson(data['$fieldName']),");
        }
      } else {
        if(fieldData.isAList) {
          functionReturn
              .add("      ${fieldName.paramify()}: ${listOf(fieldType, fieldName)},");
        } else {
          ...
          functionReturn
              .add("      ${fieldName.paramify()}: data['$fieldName'],");
        }
      }
    });
  ...
  void generateToMap() {
    ...
    classData.fieldData.forEach((fieldData) {
      ...
      if (fieldData.isAClass) {
        if (fieldData.isAList) {
          functionReturn.add(
              "      '$fieldName': ${fieldName.paramify()}.map((e) => e.toMap()).toList(),");
        } else {
          functionReturn
              .add("      '$fieldName': ${fieldName.paramify()}.toMap(),");
        }
      } else {
  ...
  void generateMakeWithDataMethod() {
    ...
    List<String> head = [
      "  static ${classData.name} makeWithData(List<String> iterations) {"
    ];
    ...
    classData.fieldData.forEach((fieldData) {
      ...
      if (fieldData.isAList && fieldData.isAClass) {
        returnBlock.add(
            "     ${name.paramify()}: $type.makeMultipleWithData(iterations),");
      } else if (fieldData.isAList && !fieldData.isAClass) {
        returnBlock
            .add("        ${name.paramify()}: ${returnBasicValue(type, name.paramify(), classData.name, fieldData.isAList)},");
      } else if (fieldData.isAClass && !fieldData.isAList) {
        returnBlock
            .add("     ${name.paramify()}: $type.makeWithData(iterations),");
      } else {
          returnBlock
              .add("     ${name.paramify()}: ${returnBasicValue(type, name.paramify(), classData.name, fieldData.isAList)},");
      }
  ...
  void generateMakeMultipleWithDataMethod() {
    ...
    List<String> head = [
      "  static List<$className> makeMultipleWithData(List<String> iterations) {"
    ];
    ...
    classData.fieldData.forEach((fieldData) {
      ...
      var name = fieldData.name;
      if (fieldData.isAList && fieldData.isAClass) {
        instantiationFields.add(
            "        ${name.paramify()}: $type.makeMultipleWithData(iterations),");
      } else if (fieldData.isAList && !fieldData.isAClass) {
        instantiationFields
            .add("        ${name.paramify()}: ${returnBasicValue(type, name.paramify(), className, fieldData.isAList)},");
      } else if (fieldData.isAClass && !fieldData.isAList) {
        instantiationFields
            .add("        ${name.paramify()}: $type.makeWithData(iterations),");
      } else {
          instantiationFields.add(
              "        ${name.paramify()}: ${returnBasicValue(type, name.paramify(), className, fieldData.isAList)},");
