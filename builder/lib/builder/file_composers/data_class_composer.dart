...
class DataClassComposer {
  ...
  void writeToFile(String pathToFile) {
    String fileString = "";
    for (var line in [...imports, ...head, ...fields, ...constructor, ...fromJsonConstructor, ...toMapMethod, ...makeWithData, ...makeMultipleWithData, ...end]) {
      fileString += ('$line\n');
    }
  ...
  void generateImports() {
    if (classData.neededImports.isNotEmpty) {
      for (var neededImport in classData.neededImports) {
  ...
  void generateFields() {
    fields.add("  // fields");
    for (var fieldData in classData.fieldData) {
